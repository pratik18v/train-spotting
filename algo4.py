...
#Auxillary functions ##########################################################
def sim_measure(stem, vec1, vec2, i, j):
...
#Part 0: Find horizontal boundaries ###########################################
def horz_bndry(folder1):
    ...
    end_time = time.time()
    print 'DONE'
    dur0 = end_time - start_time
    print 'Duration: {} sec'.format(dur0)
    
    return dur0, m, n, horz_bndry_1, horz_bndry_2
###############################################################################

#Part 1: Background subtraction ###############################################
def back_sub(stem, folder2, bs_N, bs_thresh, horz_bndry_1, horz_bndry_2):
    ...
    for i in tqdm.tqdm(range(bs_N, num_files)):
        diff = np.abs(bkg_im - \
...
def check_direc(folder2, n, hist_length):
    start_time = time.time()
    fnums = []
    ...
    return dur2, direc, fnums
###############################################################################

#Part 3: Alignment ############################################################
def align(stem, folder2, hist_length, union_length, fnums, direc, N, w, \
          smooth_coeff, lookahead, delta, intensity_thresh, start_idx):
    start_time = time.time()
    final_shifts = []
    pred_shifts = [0]
    ...
    print folder2 + 'frame' + str(fnums[start_idx]) + '.jpg', 0
    bkg = cv2.imread(folder2 + 'frame' + str(fnums[start_idx]) + '.jpg', 0)
    row_hist = np.sum(bkg, axis=0) / 255
    row_hist = row_hist[:hist_length]
    row_hist[row_hist[:] > intensity_thresh] = intensity_thresh
    row_hist[row_hist[:] < intensity_thresh] = 0
    row_hist = medfilt(row_hist, smooth_coeff)
    ...
    for i in tqdm.tqdm(range(start_idx+1,len(fnums))):
        ...
        curr_hist = medfilt(curr_hist, smooth_coeff)
        ...
        for j in range(-w, w):
            ...
            if start_pt == 0 and len(prev_hist) != hist_length:
                corr = sim_measure(stem, curr_hist[-len(prev_hist):], prev_hist, i, j+100)
            elif end_pt == union_length and len(prev_hist) != hist_length:
                corr = sim_measure(stem, curr_hist[:len(prev_hist)], prev_hist, i, j+100)
            else:
                corr = sim_measure(stem, curr_hist, prev_hist, i, j+100)
...
#            break
    ...
    str_version = 'N-{}_w-{}_ith-{}_sth-{}_scoeff-{}_la-{}_del-{}_sidx-{}'\
                     .format(N,w,intensity_thresh,shift_thresh, smooth_coeff, \
                             lookahead, delta, start_idx)
    end_time = time.time()
    dur3 = end_time - start_time
    print 'Duration: {}'.format(dur3)
    print 'DONE'
    
    return dur3, str_version, hist_union, final_shifts, pred_shifts, start, end
###############################################################################

#Part 5: Finding minimas ######################################################
def find_min(stem, graph_dir, str_version, hist_union, smooth_window, \
             lookahead, delta):
    start_time = time.time()
    car_lengths = []
    ...
    return dur4, smooth_union, initial_pred_gaps, pred_gaps, yn, gnd_truth
###############################################################################

#Part 6: Cropping #############################################################
def gen_crops(stem, fnums, direc, pred_gaps, start, end, \
              horz_bndry_1, horz_bndry_2, n):
    ...
    return dur5, req_frames, car_lengths
###############################################################################

#Part 7: Ground truth analysis ################################################
def gt_analysis(stem, graph_dir, fnums, str_version, start, smooth_union, pred_gaps, \
                initial_pred_gaps, yn, gnd_truth):
    initial_error_1 = []
    initial_error_2 = []
    error_1 = []
    error_2 = []
    ...
    return dur6, initial_error_1, initial_error_2, error_1, error_2
###############################################################################

#Part 4: Graphs ###############################################################
def gen_graphs(stem, graph_dir, hist_union, final_shifts, pred_shifts, car_lengths, \
               str_version, error_1, error_2):
    fig = plt.figure(figsize=(20, 4))
    ax = fig.add_subplot(111)
    ax.grid()
    plt.figure(figsize=(20,4))
...
def gen_html(stem, graph_dir, video_name, video_link, fnums, m, n, pred_gaps, \
             final_shifts, str_version, dur0, dur1, dur2, dur3, dur4, dur5, dur6, \
             initial_error_1, initial_error_2, error_1, error_2, req_frames, car_lengths):
    ...
    time_str = '<p>Time elapsed in function find_horz_boundaries is: {} s</p>'.format(dur0)
    time_str = '<p>Time elapsed in function back_sub is: {} s</p>'.format(dur1)
    ...
    total_time = dur0 + dur1 + dur2 + dur3 + dur4 + dur5 + dur6
...
if __name__ == '__main__':
    ...
    #2. BB_NS_WB.MOV : https://drive.google.com/open?id=0B8EK0LxdsI4qdjJhR0NGaUt1eXc
    #3. BB_NS_mixed_freight.MOV : https://drive.google.com/open?id=0B8EK0LxdsI4qYUpxYzBKY05kSVE
    #4. train_crossing_3.mp4 : https://drive.google.com/open?id=0B3_7z0wYZNLQMnpvUHowNmFpcUU
    #5. train_crossing_4.mp4 : https://drive.google.com/open?id=0B3_7z0wYZNLQS3d2TW5mTFQwU0E
    configs = {}    
    configs['video_name'] = 'BB_NS_WB.MOV'
    configs['video_link'] = 'https://drive.google.com/open?id=0B8EK0LxdsI4qdjJhR0NGaUt1eXc'
    configs['stem'] = configs['video_name'].split('.')[0] + '/'
    configs['folder1'] = configs['stem'] + 'opt_flow/'
    configs['folder2'] = configs['stem'] + 'new_backsub/'
    configs['graph_dir'] = 'graphs/'
    configs['hist_length'] = 1920
    configs['union_length'] = int(2.5e5)
    configs['bs_N'] = 10
    configs['bs_thresh'] = 30
    configs['start_idx'] = 0
    configs['N'] = 5
    configs['w'] = 100
    configs['intensity_thresh'] = 30
    configs['smooth_coeff'] = 11
    configs['smooth_window'] = 5
    configs['lookahead'] = 5
    configs['delta'] = 5
    ###########################################################################

    dur0, m, n, horz_bndry_1, horz_bndry_2 = horz_bndry(configs['folder1'])
#
#    dur1 = back_sub(configs['stem'], configs['folder2'], configs['bs_N'], \
#                    configs['bs_thresh'], horz_bndry_1, horz_bndry_2)
#
    dur1 = 300

    dur2, direc, fnums = check_direc(configs['folder2'], n, configs['hist_length'])

    dur3, str_version, hist_union, final_shifts, pred_shifts, start, end = \
        align(configs['stem'], configs['folder2'], configs['hist_length'], configs['union_length'], \
              fnums, direc, configs['N'], configs['w'],  configs['smooth_coeff'], \
              configs['lookahead'], configs['delta'], configs['intensity_thresh'], \
              configs['start_idx'])

    dur4, smooth_union, initial_pred_gaps, pred_gaps, yn, gnd_truth = \
        find_min(configs['stem'], configs['graph_dir'], str_version, hist_union, \
                 configs['smooth_window'], configs['lookahead'], configs['delta'])

    dur5, req_frames, car_lengths = \
        gen_crops(configs['stem'], fnums, direc, pred_gaps, start, end, \
                  horz_bndry_1, horz_bndry_2, n)

    dur6, initial_error_1, initial_error_2, error_1, error_2 = \
        gt_analysis(configs['stem'], configs['graph_dir'], fnums, str_version, \
                    start, smooth_union, pred_gaps, initial_pred_gaps, yn, gnd_truth)
    
    gen_graphs(configs['stem'], configs['graph_dir'], hist_union, final_shifts, \
               pred_shifts, car_lengths, str_version, error_1, error_2)

    gen_html(configs['stem'], configs['graph_dir'], configs['video_name'], \
             configs['video_link'], fnums, m, n, pred_gaps, final_shifts, \
             str_version, dur0, dur1, dur2, dur3, dur4, dur5, dur6, \
             initial_error_1, initial_error_2, error_1, error_2, req_frames, car_lengths)